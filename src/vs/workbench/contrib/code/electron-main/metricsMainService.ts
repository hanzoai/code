...
export class MetricsMainService extends Disposable implements IMetricsService {
	...
	// helper - looks like this is stored in a .vscdb file in ~/Library/Application Support/Code
	...
	async initialize() {
		...
		const { commit, version, codeVersion, release, quality } = this._productService
		...
		console.log('User is opted out of basic Code metrics?', didOptOut)
		if (didOptOut) {
			this.client.optOut()
		}
		else {
		...
		console.log('Code posthog metrics info:', JSON.stringify(identifyMessage, null, 2))
	}
