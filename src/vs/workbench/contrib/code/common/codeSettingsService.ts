...
import { CODE_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, CodeStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './codeSettingsTypes.js';
...
// type RealCodeSettings = Exclude<keyof CodeSettingsState, '_modelOptions'>
// type EventProp<T extends RealCodeSettings = RealCodeSettings> = T extends 'globalSettings' ? [T, keyof CodeSettingsState[T]] : T | 'all'


export interface ICodeSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: CodeSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: CodeSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: CodeStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, CodeStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: CodeSettingsState): CodeSettingsState => {
...
const _validatedModelState = (state: Omit<CodeSettingsState, '_modelOptions'>): CodeSettingsState => {
...
export const ICodeSettingsService = createDecorator<ICodeSettingsService>('CodeSettingsService');
class CodeSettingsService extends Disposable implements ICodeSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: CodeSettingsState;
	...
	dangerousSetState = async (newState: CodeSettingsState) => {
	...
	private async _readState(): Promise<CodeSettingsState> {
		const encryptedState = this._storageService.get(CODE_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(CODE_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: CodeSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: CodeStatefulModelInfo[] = [
...
registerSingleton(ICodeSettingsService, CodeSettingsService, InstantiationType.Eager);
