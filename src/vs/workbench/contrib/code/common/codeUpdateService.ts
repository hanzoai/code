...
import { CodeCheckUpdateRespose } from './codeUpdateServiceTypes.js';



export interface ICodeUpdateService {
	readonly _serviceBrand: undefined;
	check: (explicit: boolean) => Promise<CodeCheckUpdateRespose>;
}
...
export class CodeUpdateService implements ICodeUpdateService {
	...
	check: ICodeUpdateService['check'] = async (explicit) => {
		const res = await this.codeUpdateService.check(explicit)
		return res
	}
}
