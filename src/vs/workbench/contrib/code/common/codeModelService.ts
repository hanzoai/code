...
export interface ICodeModelService {
	readonly _serviceBrand: undefined;
	initializeModel(uri: URI): Promise<void>;
	getModel(uri: URI): CodeModelType;
	getModelFromFsPath(fsPath: string): CodeModelType;
	getModelSafe(uri: URI): Promise<CodeModelType>;
	saveModel(uri: URI): Promise<void>;

}

export const ICodeModelService = createDecorator<ICodeModelService>('codeCodeModelService');

class CodeModelService extends Disposable implements ICodeModelService {
	_serviceBrand: undefined;
	static readonly ID = 'codeCodeModelService';
	...
	getModelSafe = async (uri: URI): Promise<CodeModelType> => {
...
registerSingleton(ICodeModelService, CodeModelService, InstantiationType.Eager);
