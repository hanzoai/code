...
import { ICodeSettingsService } from './codeSettingsService.js';
import { ILLMMessageService } from './sendLLMMessageService.js';
import { Emitter, Event } from '../../../../base/common/event.js';
import { Disposable, IDisposable } from '../../../../base/common/lifecycle.js';
import { RefreshableProviderName, refreshableProviderNames, SettingsOfProvider } from './codeSettingsTypes.js';
...
export class RefreshModelService extends Disposable implements IRefreshModelService {
	...
	constructor(
		...
		const initializeAutoPollingAndOnChange = () => {
			...
			if (!codeSettingsService.state.globalSettings.autoRefreshModels) return

			for (const providerName of refreshableProviderNames) {

				// const { '_didFillInProviderSettings': enabled } = this.codeSettingsService.state.settingsOfProvider[providerName]
				this.startRefreshingModels(providerName, autoOptions)

				// every time providerName.enabled changes, refresh models too, like a useEffect
				let relevantVals = () => refreshBasedOn[providerName].map(settingName => codeSettingsService.state.settingsOfProvider[providerName][settingName])
				let prevVals = relevantVals() // each iteration of a for loop has its own context and vars, so this is ok
				disposables.add(
					codeSettingsService.onDidChangeState(() => { // we might want to debounce this
		...
		codeSettingsService.waitForInitState.then(() => {
			initializeAutoPollingAndOnChange()
			this._register(
				codeSettingsService.onDidChangeState((type) => { if (typeof type === 'object' && type[1] === 'autoRefreshModels') initializeAutoPollingAndOnChange() })
			)
		})
	...
	startRefreshingModels: IRefreshModelService['startRefreshingModels'] = (providerName, options) => {
		...
		const autoPoll = () => {
			if (this.codeSettingsService.state.globalSettings.autoRefreshModels) {
				// resume auto-polling
		...
		listFn({
			...
			onSuccess: ({ models }) => {
				...
				this.codeSettingsService.setAutodetectedModels(
					providerName,
					models.map(model => {
				...
				if (options.enableProviderOnSuccess) this.codeSettingsService.setSettingOfProvider(providerName, '_didFillInProviderSettings', true)
