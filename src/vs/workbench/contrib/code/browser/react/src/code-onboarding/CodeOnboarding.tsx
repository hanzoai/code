...
import { displayInfoOfProviderName, ProviderName, providerNames, localProviderNames, featureNames, FeatureName, isFeatureNameDisabled } from '../../../../common/codeSettingsTypes.js';
import { ChatMarkdownRender } from '../markdown/ChatMarkdownRender.js';
import { OllamaSetupInstructions, OneClickSwitchButton, SettingsForProvider, ModelDump } from '../code-settings-tsx/Settings.js';
...
export const CodeOnboarding = () => {
	...
	const isOnboardingComplete = codeSettingsState.globalSettings.isOnboardingComplete || OVERRIDE_VALUE
			...
			{providerNamesOfTab[currentTab].map((providerName) => (
				<div key={providerName} className="w-full max-w-xl mb-10">
					<div className="text-xl mb-2">
						...
						{providerName === 'gemini' && (
							<span
								data-tooltip-id="code-tooltip-provider-info"
...
// 		title:
// 			div
// 				"How would you like to use Code?"
// 		content:
...
const CodeOnboardingContent = () => {


	const accessor = useAccessor()
	const codeSettingsService = accessor.get('ICodeSettingsService')
	const codeMetricsService = accessor.get('IMetricsService')
	...
	const didFillInProviderSettings = selectedProviderName && codeSettingsState.settingsOfProvider[selectedProviderName]._didFillInProviderSettings
	const isApiKeyLongEnoughIfApiKeyExists = selectedProviderName && codeSettingsState.settingsOfProvider[selectedProviderName].apiKey ? codeSettingsState.settingsOfProvider[selectedProviderName].apiKey.length > 15 : true
	const isAtLeastOneModel = selectedProviderName && codeSettingsState.settingsOfProvider[selectedProviderName].models.length >= 1
	...
	const lastPagePrevAndNextButtons = <div className="max-w-[600px] w-full mx-auto flex flex-col items-end">
		<div className="flex items-center gap-2">
			...
			<PrimaryActionButton
				onClick={() => {
					codeSettingsService.setGlobalSetting('isOnboardingComplete', true);
					codeMetricsService.capture('Completed Onboarding', { selectedProviderName, wantToUseOption })
				}}
				ringSize={codeSettingsState.globalSettings.isOnboardingComplete ? 'screen' : undefined}
			>Enter the Code</PrimaryActionButton>
		</div>
	</div>
	...
	useEffect(() => {
		if (!codeSettingsState.globalSettings.isOnboardingComplete) {
			setPageIndex(0)
		}
	}, [setPageIndex, codeSettingsState.globalSettings.isOnboardingComplete])
		...
		0: <OnboardingPageShell
			content={
				<div className='flex flex-col items-center gap-8'>
					<div className="text-5xl font-light text-center">Welcome to Code</div>
