...
import '../styles.css'
import { CODE_CTRL_K_ACTION_ID, CODE_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { Circle, MoreVertical } from 'lucide-react';
import { useEffect, useState } from 'react';

import { CodeSelectionHelperProps } from '../../../../../../contrib/code/browser/codeSelectionHelperWidget.js';
import { CODE_OPEN_SETTINGS_ACTION_ID } from '../../../codeSettingsPane.js';


export const CodeSelectionHelperMain = (props: CodeSelectionHelperProps) => {
	...
	>
		<CodeSelectionHelper {...props} />
	</div>
}



const CodeSelectionHelper = ({ rerenderKey }: CodeSelectionHelperProps) => {
	...
	const ctrlLKeybind = keybindingService.lookupKeybinding(CODE_CTRL_L_ACTION_ID)
	const ctrlKKeybind = keybindingService.lookupKeybinding(CODE_CTRL_K_ACTION_ID)
	...
	useEffect(() => {
		const disposable = commandService.onWillExecuteCommand(e => {
			if (e.commandId === CODE_CTRL_L_ACTION_ID || e.commandId === CODE_CTRL_K_ACTION_ID) {
	...
	const defaultHTML = <>
		{ctrlLKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(CODE_CTRL_L_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
		...
		{ctrlKKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(CODE_CTRL_K_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
	...
	const moreOptionsHTML = <>
		<div
			...
			'
			onClick={() => {
				commandService.executeCommand(CODE_OPEN_SETTINGS_ACTION_ID);
				setClickState('clickedOption');
			}}
		>
			Disable Suggestions?
