...
import { CodeCommandBarProps } from '../../../codeCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	CODE_GOTO_NEXT_DIFF_ACTION_ID,
	CODE_GOTO_PREV_DIFF_ACTION_ID,
	CODE_GOTO_NEXT_URI_ACTION_ID,
	CODE_GOTO_PREV_URI_ACTION_ID,
	CODE_ACCEPT_FILE_ACTION_ID,
	CODE_REJECT_FILE_ACTION_ID,
	CODE_ACCEPT_ALL_DIFFS_ACTION_ID,
	CODE_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const CodeCommandBarMain = ({ uri, editor }: CodeCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@void-scope ${isDark ? 'dark' : ''}`}
	>
		<CodeCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const CodeCommandBar = ({ uri, editor }: CodeCommandBarProps) => {
	...
	const codeModelService = accessor.get('ICodeModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(CODE_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(CODE_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(CODE_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(CODE_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(CODE_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(CODE_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(CODE_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(CODE_REJECT_ALL_DIFFS_ACTION_ID);
