...
import { mountCodeOnboarding } from './react/out/code-onboarding/index.js'
...
export class OnboardingContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'workbench.contrib.codeOnboarding';
	...
	private initialize(): void {
		...
		if (workbench) {
			...
			this.instantiationService.invokeFunction((accessor: ServicesAccessor) => {
				const result = mountCodeOnboarding(onboardingContainer, accessor);
