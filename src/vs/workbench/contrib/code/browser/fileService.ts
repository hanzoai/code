...
class FilePromptActionService extends Action2 {
	private static readonly CODE_COPY_FILE_PROMPT_ID = 'void.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.CODE_COPY_FILE_PROMPT_ID,
			title: localize2('codeCopyPrompt', 'Code: Copy Prompt'),
			menu: [{
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await codeModelService.getModelSafe(uri)).model?.getLanguageId() || '',
