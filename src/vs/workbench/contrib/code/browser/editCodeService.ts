...
import { codePrefixAndSuffix, ctrlKStream_userMessage, ctrlKStream_systemMessage, defaultQuickEditFimTags, rewriteCode_systemMessage, rewriteCode_userMessage, searchReplaceGivenDescription_systemMessage, searchReplaceGivenDescription_userMessage, tripleTick, } from '../common/prompt/prompts.js';
import { ICodeCommandBarService } from './codeCommandBarService.js';
import { IKeybindingService } from '../../../../platform/keybinding/common/keybinding.js';
import { CODE_ACCEPT_DIFF_ACTION_ID, CODE_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { FeatureName } from '../common/codeSettingsTypes.js';
import { ICodeModelService } from '../common/codeModelService.js';
import { deepClone } from '../../../../base/common/objects.js';
import { acceptBg, acceptBorder, buttonFontSize, buttonTextColor, rejectBg, rejectBorder } from '../common/helpers/colors.js';
import { DiffArea, Diff, CtrlKZone, CodeFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js';
// import { isMacintosh } from '../../../../base/common/platform.js';
// import { CODE_OPEN_SETTINGS_ACTION_ID } from './codeSettingsPane.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._codeModelService.initializeModel(model.uri)
	...
	// 				enabled: true,
	// 				label: `Open Code's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(CODE_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'code-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'code-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'code-highlightBG')
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'code-greenBG', {
	...
	private _getCurrentCodeFileSnapshot = (uri: URI): CodeFileSnapshot => {
		const { model } = this._codeModelService.getModel(uri)
	...
	private _restoreCodeFileSnapshot = async (uri: URI, snapshot: CodeFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: CodeFileSnapshot = this._getCurrentCodeFileSnapshot(uri)
		let afterSnapshot: CodeFileSnapshot | null = null
		...
		const elt: IUndoRedoElement = {
			...
			undo: async () => { opts?.onWillUndo?.(); await this._restoreCodeFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restoreCodeFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentCodeFileSnapshot(uri)
			await this._codeModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}


	public getCodeFileSnapshot(uri: URI) {
		return this._getCurrentCodeFileSnapshot(uri)
	}


	public restoreCodeFileSnapshot(uri: URI, snapshot: CodeFileSnapshot): void {
		this._restoreCodeFileSnapshot(uri, snapshot)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Code: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._codeModelService.initializeModel(uri)
		await this._codeModelService.saveModel(uri) // save the URI
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Code: diff.type not recognized on: ${from}`)
		}
		...
		if (from === 'ClickApply') {
			...
			const { prefix, suffix } = codePrefixAndSuffix({ fullFileStr: originalFileCode, startLine, endLine })
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Code: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`Code error: ${diff}.type not recognized`)
		}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@ICodeCommandBarService private readonly _codeCommandBarService: ICodeCommandBarService,
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(CODE_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(CODE_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._codeCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._codeCommandBarService.onDidChangeState(e => {
