...
import { mountCodeCommandBar } from './react/out/code-editor-widgets-tsx/index.js'
...
import { CODE_ACCEPT_DIFF_ACTION_ID, CODE_REJECT_DIFF_ACTION_ID, CODE_GOTO_NEXT_DIFF_ACTION_ID, CODE_GOTO_PREV_DIFF_ACTION_ID, CODE_GOTO_NEXT_URI_ACTION_ID, CODE_GOTO_PREV_URI_ACTION_ID, CODE_ACCEPT_FILE_ACTION_ID, CODE_REJECT_FILE_ACTION_ID, CODE_ACCEPT_ALL_DIFFS_ACTION_ID, CODE_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface ICodeCommandBarService {
...
export const ICodeCommandBarService = createDecorator<ICodeCommandBarService>('CodeCommandBarService');
...
export class CodeCommandBarService extends Disposable implements ICodeCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'void.CodeCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._codeModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(ICodeCommandBarService, CodeCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type CodeCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountCodeCommandBar(root, accessor, { uri, editor } satisfies CodeCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies CodeCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODE_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('codeAcceptDiffAction', 'Code: Accept Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODE_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('codeRejectDiffAction', 'Code: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODE_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('codeGoToNextDiffAction', 'Code: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODE_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('codeGoToPrevDiffAction', 'Code: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODE_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('codeGoToNextUriAction', 'Code: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODE_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('codeGoToPrevUriAction', 'Code: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODE_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('codeAcceptFileAction', 'Code: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODE_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('codeRejectFileAction', 'Code: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODE_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('codeAcceptAllDiffsAction', 'Code: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODE_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('codeRejectAllDiffsAction', 'Code: Reject All Diffs in All Files'),
			keybinding: {
