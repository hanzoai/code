...
import { mountCodeSelectionHelper } from './react/out/code-editor-widgets-tsx/index.js';
...
export type CodeSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	public static readonly ID = 'editor.contrib.codeSelectionHelper';
	// react
	...
	constructor(
		...
		@ICodeSettingsService private readonly _codeSettingsService: ICodeSettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountCodeSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._codeSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies CodeSelectionHelperProps)
