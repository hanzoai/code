...
// gets converted to --vscode-code-greenBG, see void.css, asCssVariable
...
class InlineDiffsService extends Disposable implements IInlineDiffsService {
	...
	private async _writeText(uri: URI, text: string, range: IRange, { shouldRealignDiffAreas }: { shouldRealignDiffAreas: boolean }) {
		...
		// const edits = this.worker.$Code_computeMoreMinimalEdits(uri.toString(), [{ range, text }], false)
	...
	private _initializeStartApplying(opts: StartApplyingOpts): DiffZone | undefined {
		...
		if (featureName === 'Ctrl+L') {
			...
			throw new Error(`Code: diff.type not recognized on: ${featureName}`)
		}
		...
		const isOllamaFIM = false // this._codeSettingsService.state.modelSelectionOfFeature['Ctrl+K']?.providerName === 'ollama'
...
// 		super({
// 			id: 'void.testDiff',
// 			title: localize2('codeTestDiff', 'Code Test Diff'),
// 			f1: true,
// 		});
// 	}
