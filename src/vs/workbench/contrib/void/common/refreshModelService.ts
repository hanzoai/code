	...
	constructor(
		...
		const initializeAutoPollingAndOnChange = () => {
			...
			for (const providerName of refreshableProviderNames) {
				...
				// const { _enabled: enabled } = this.codeSettingsService.state.settingsOfProvider[providerName]
	...
	startRefreshingModels: IRefreshModelService['startRefreshingModels'] = (providerName, options) => {
		...
		listFn({
			...
			onSuccess: ({ models }) => {
				...
				if (options.enableProviderOnSuccess) this.codeSettingsService.setSettingOfProvider(providerName, '_enabled', true)
